...
void AlbumManager::showPicture()
{
	...
	do
	{
		std::cout << "Pick how to open:" << std::endl;
		std::cout << "1. Paint" << std::endl;
		std::cout << "2. Irfanview" << std::endl;
		std::cin >> opt;
	} while (opt != 1 && opt != 2);
	if (opt == 1)
	{
	...
	{
		std::cout << "Irfanview is currently not supported please try again later." << std::endl;
		std::cout << "Have MSpaint until we fix this :)" << std::endl;
		msPaint = "C:\\WINDOWS\\system32\\mspaint.exe";
	}
	LPCSTR msPaintC = const_cast<char*>(msPaint.c_str());
	ZeroMemory(&(si), sizeof(si));
	si.cb = sizeof(si);
	ZeroMemory(&(pi), sizeof(pi));

	INT size = strlen(msPaintC) + path.length() + 2;
	size = size + 2;
	PCHAR param = (PCHAR)malloc(size * sizeof(CHAR));
	sprintf_s(param, size, "%s %s", msPaintC, path.c_str());
	bool imgProc = CreateProcessA(msPaintC, param, NULL, NULL, FALSE, 0, NULL, NULL, &si, &pi);
	free(param);
}
